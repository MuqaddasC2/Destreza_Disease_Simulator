        ...
        case 'init':
          ...
          simulation = d3.forceSimulation(nodes)
            ...
            .on('tick', () => {
              ...
              self.postMessage({ type: 'tick', data: positions }, [positions.buffer]);
            });
          break;
...
export function ForceGraphGPU({
  ...
  const computeForcesRef = useRef<any>(null)
  ...
  useEffect(() => {
    ...
    return () => {
      if (gpuRef.current) {
        ...
        gpuRef.current = null
        computeForcesRef.current = null
      }
    }
  }, [])

  // Worker initialization - only depends on useCanvas and useGPU
useEffect(() => {
    ...
    if (type === "tick") {
      setNodes((prevNodes) => {
        ...
        for (let i = 0; i < newNodes.length; i++) {
          newNodes[i].x = data[i * 2];
          newNodes[i].y = data[i * 2 + 1];
        }
        return newNodes;
      });
    }
...
}, [useCanvas, useGPU]); // Only these dependencies

  // Handle canvas rendering
  const renderCanvas = useCallback(throttle(() => {
    ...
    const nodeRadius = Math.max(1, Math.min(5, zoomLevel * 2))
    const drawDetailed = zoomLevel > 0.5

    // Create a flat lookup array for nodes by id
    const nodeById = new Array(nodes.length)
    nodes.forEach((node) => {
    ...
    if (drawDetailed) {
      context.beginPath()
      links.forEach((link) => {
        const source = typeof link.source === "number" ? nodeById[link.source] : (link.source as Node)
        const target = typeof link.target === "number" ? nodeById[link.target] : (link.target as Node)
      ...
      // When zoomed out, just draw a simplified network structure
      context.beginPath()
      context.strokeStyle = "#616083"
      context.globalAlpha = 0.2
      context.lineWidth = 0.5

      // Only draw a subset of links when zoomed out
      const linkStep = Math.max(1, Math.floor(links.length / 1000))
    ...
    if (nodes.length > 5000 && zoomLevel < 0.3) {
      // For very large networks when zoomed out, draw a density heatmap
      const cellSize = 20
      const grid = new Map()
      ...
      // Draw each cell as a circle with size proportional to count
      grid.forEach((cell) => {
        const radius = Math.sqrt(cell.count) * 2

        // Determine dominant status in this cell
        let dominantStatus = "susceptible"
        let maxCount = cell.susceptible

        if (cell.exposed > maxCount) {
          dominantStatus = "exposed"
          maxCount = cell.exposed
        }
        if (cell.infectious > maxCount) {
          dominantStatus = "infectious"
          maxCount = cell.infectious
        }
        if (cell.recovered > maxCount) {
          dominantStatus = "recovered"
          maxCount = cell.recovered
        }
        if (cell.deceased > maxCount) {
          dominantStatus = "deceased"
          maxCount = cell.deceased
        }

        // Set fill color based on dominant status
        if (dominantStatus === "susceptible")
          context.fillStyle = "#3B82F6" // Blue
        else if (dominantStatus === "exposed")
          context.fillStyle = "#FF81FF" // Pink
        else if (dominantStatus === "infectious")
          context.fillStyle = "#FF5555" // Red
        else if (dominantStatus === "recovered")
          context.fillStyle = "#51FAAA" // Green
        else if (dominantStatus === "deceased") context.fillStyle = "#A855F7" // Purple

        context.beginPath()
        context.arc(cell.x, cell.y, radius, 0, 2 * Math.PI)
        context.fill()

        // Add count text for larger clusters
        if (radius > 10) {
          context.fillStyle = "#FFFFFF"
          context.font = "10px Arial"
          context.textAlign = "center"
          context.textBaseline = "middle"
          context.fillText(cell.count.toString(), cell.x, cell.y)
        }
      })
    } else {
  ...
  const generateNetwork = () => {
    ...
    // Initialize GPU.js kernel if using GPU
    if (useGPU && gpuRef.current && newNodes.length > 0) {
      try {
        // Create a GPU.js kernel for force calculation
        computeForcesRef.current = gpuRef.current
          .createKernel(function (xs: number[], ys: number[]) {
            const xi = xs[this.thread.x]
            const yi = ys[this.thread.x]
            let fx = 0,
              fy = 0

            // Calculate repulsive forces from all other nodes
            for (let j = 0; j < this.constants.N; j++) {
              if (j === this.thread.x) continue

              const dx = xi - xs[j]
              const dy = yi - ys[j]
              const d2 = dx * dx + dy * dy + 0.001 // Add small epsilon to avoid division by zero
          ...
          .setConstants({ N: newNodes.length })
          .setOutput([newNodes.length])
      } catch (error) {
        console.error("Failed to create GPU.js kernel:", error)
        setUseGPU(false)
      }
    }
  }
  ...
  const updateStats = (currentNodes: Node[]) => {
    ...
    const isComplete = newStats.exposed === 0 && newStats.infectious === 0

    return { stats: newStats, isComplete }
  }
  ...
  const simulationStep = () => {
    ...
    setNodes((prevNodes) => {
      ...
      const { stats: newStats, isComplete } = updateStats(nodes)
    ...
    // Apply GPU-accelerated forces if enabled
    if (useGPU && computeForcesRef.current && nodePositionsRef.current) {
      try {
        // Extract x and y positions into separate arrays for GPU processing
        const xPositions = new Float32Array(nodes.length)
        const yPositions = new Float32Array(nodes.length)
        ...
        // Compute forces using GPU
        const forces = computeForcesRef.current(xPositions, yPositions)
        ...
        for (let i = 0; i < nodes.length; i++) {
          ...
          if (node.x !== undefined && node.y !== undefined) {
            // Scale force to avoid extreme movements
            const scale = 0.1
            node.x += force[0] * scale
            node.y += force[1] * scale

            // Update positions in the Float32Array for efficient transfer
            nodePositionsRef.current[i * 2] = node.x
            nodePositionsRef.current[i * 2 + 1] = node.y
          }
        }
      } catch (error) {
  ...
  const toggleGPU = (enabled: boolean) => {
    ...
    // Reinitialize GPU kernel if needed
    if (enabled && gpuRef.current && nodes.length > 0) {
      try {
        // Create a GPU.js kernel for force calculation
        computeForcesRef.current = gpuRef.current
          .createKernel(function (xs: number[], ys: number[]) {
          ...
          .setConstants({ N: nodes.length })
          .setOutput([nodes.length])
      } catch (error) {
        console.error("Failed to create GPU.js kernel:", error)
        setUseGPU(false)
      }
    ...
    if (canvasRef.current) {
      ...
      const resizeCanvas = () => {
        if (canvasRef.current && containerRef.current) {
          ...
          // Get context and scale all drawing operations by DPR
  ...
  useEffect(() => {
    ...
    // Early-exit throttle: Stop simulation when it cools down
    const checkAlpha = () => {
      if (simulation.alpha() < 0.005) {
        simulation.stop()
      } else {
        setTimeout(checkAlpha, 1000)
      }
    }
        ...
        <GlassCard className="lg:col-span-3">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-2">
                <span className="text-sm text-[#616083]">GPU Acceleration</span>
                <Switch
                  checked={useGPU}
                  onCheckedChange={toggleGPU}
                  id="gpu-mode"
                  disabled={!useCanvas} // Only enable when canvas mode is active
                />
                <Cpu className="h-4 w-4 text-[#616083]" />
              </div>
